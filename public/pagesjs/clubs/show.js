/**
 * Configuración de DataTables para eventos asignados a un club
 */
...
const initAssignedEventsTable = () => {
    ...
    // Obtener el ID del club de la URL
    const clubId = window.location.pathname.split('/')[2];

    dataTable.DataTable({
        processing: true,
        ajax: {
            url: `/clubs/${clubId}/assigned-events`,
        },
        ...
        columns: [
            ...
            { data: 'year', name: 'year' },
            { data: 'receivables_count', name: 'receivables_count' },
            { 
                data: 'actions', 
                name: 'actions', 
                orderable: false, 
        ...
        columnDefs: [
            ...
            },
            {
                ...
                searchable: true,
                render: function(data, type, row) {
                    if (row.receivables_count > 0) {
                        return `<span class="badge bg-warning">${row.receivables_count} cuenta(s)</span>`;
                    } else {
 ...
 */
const deleteEventAssignment = (clubId, eventId) => {
    Swal.fire({
        ...
        if (result.isConfirmed) {
            ...
            fetch(`/clubs/${clubId}/events/${eventId}/detach`, {
                method: 'DELETE',
                headers: {
