...
document.addEventListener('DOMContentLoaded', () => {
    initSubcategorySuppliersTable();
});

/**
 * Inicializa y configura DataTable para tipos de gastos
 */
const initSubcategorySuppliersTable = () => {
    ...
    dataTable.DataTable({
        ...
        columns: [
            { data: 'name', name: 'name' },
            { data: 'category_supplier.name', name: 'category_supplier.name' },
            { 
                data: 'actions', 
                name: 'actions', 
                orderable: false, 
...
const deleteRecord = (id) => {
    Swal.fire({
        ...
        if (result.isConfirmed) {
            window.location.href = `/subcategorias-proveedores/${id}/destroy`;
        }
    });
};