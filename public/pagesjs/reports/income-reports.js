...
class IncomeReportsManager {
    ...
    initDataTable() {
        ...
        this.datatable = $table.DataTable({
            ...
            footerCallback: function(row, data, start, end, display) {
                const api = this.api ? this.api() : new $.fn.dataTable.Api(this);

                // Usa el formateador global si existe, si no, crea uno local
                const numberFormat = (window.incomeReportsManager && window.incomeReportsManager.numberFormat)
                    ? window.incomeReportsManager.numberFormat
                    : new Intl.NumberFormat("es-MX", { minimumFractionDigits: 0 });

                // 1. Sumar montos por moneda de TODOS los datos (total general)
                let totalesGenerales = {};
                let allData = api.ajax.json() && api.ajax.json().data ? api.ajax.json().data : [];
                allData.forEach(row => {
                    const moneda = row.currency && row.currency.name ? row.currency.name : "Desconocida";
                ...
                api.rows({ search: 'applied' }).every(function () {
                    ...
                    const moneda = rowData.currency && rowData.currency.name ? rowData.currency.name : "Desconocida";
                ...
                Object.keys(totalesGenerales).forEach(moneda => {
                    ...
                    if (el) {
                        el.textContent = numberFormat.format(totalesFiltrados[moneda] || 0) ;
                    }
                });
            },
        });
    ...
    updateTotals() {
        ...
        for (const currency in currencyTotals) {
            const el = document.getElementById(`totalIngresos${currency.replace(/\s/g, "")}`);
            if (el) {
                el.textContent = this.numberFormat.format(currencyTotals[currency]);
            }
        }
    }
    ...
    setupCustomFilters() {
        ...
        setTimeout(() => {
            ...
            $('.dataTables_filter').addClass('d-flex align-items-end gap-2 mb-2').append(filtersHtml);
...
$(function () {
    window.incomeReportsManager = new IncomeReportsManager();
});
